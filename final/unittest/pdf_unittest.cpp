...
TEST(PDFTests, CosPdfGenerateTest) {
  Vector3 n(0,0,1);
  pcg32_state rng = init_pcg32(time(NULL));
  cosine_pdf* cosPdf = new cosine_pdf(n,rng);  
  auto wo = cosPdf->generate();
  
  EXPECT_NEAR(1, wo.x*wo.x+wo.y*wo.y+wo.z*wo.z, 0.0001);
  EXPECT_TRUE(0 < wo.z);
}

TEST(PDFTests, CosPdfValueTest0) {
  Vector3 n(0,0,1);
  pcg32_state rng = init_pcg32(time(NULL));
  cosine_pdf* cosPdf = new cosine_pdf(n,rng);  
...
TEST(PDFTests, CosPdfValueTest1) {
  Vector3 n(0,0,1);
  pcg32_state rng = init_pcg32(time(NULL));
  cosine_pdf* cosPdf = new cosine_pdf(n,rng);  
...
TEST(LightPDFTests, SpherelightPdfSetupTest0) {
  Sphere sph;
  ...
  EXPECT_NEAR(0, length(lp->sph->center), 0.0001);
  EXPECT_TRUE( lp->tri == nullptr );
  EXPECT_NEAR(1, lp->p.x, 0.0001);
  EXPECT_NEAR(0, lp->p.y, 0.0001);
